...
static const char* xtermname = "st";

static void
spawn_cb(Fl_Widget*, void*n)
{
  char* name = (char*)n;
  // Código para evitar procesos zombis
  if (fork() == 0) {
    if (fork() == 0) {
      close(ConnectionNumber(fl_display));
      // Usamos sh -c para que pueda interpretar "rofi -show drun" como un comando completo
      execlp("sh", "sh", "-c", name, (char *)NULL);
...
#if XTERM_MENU_ITEM
  {"Nueva st", 0, spawn_cb, (void*)xtermname, 0, 0, 0, MENU_FONT_SIZE},
#endif
  {"Rofi", 0, spawn_cb, (void*)"rofi -show drun", 0, 0, 0, MENU_FONT_SIZE}, 
#if DESKTOPS
  {"Nuevo desktop", 0, new_desktop_cb, 0, 0, 0, 0, MENU_FONT_SIZE},
#endif

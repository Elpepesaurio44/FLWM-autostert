...
static void initialize() {
  ...
  // --- TEMA NORD (Azul Ártico) ---
  // Nord0: Reemplaza el gris por el azul oscuro de fondo
  Fl::set_color(FL_GRAY, 0x2e344000); 

  // Nord4: Color del texto (Blanco nieve/Gris muy claro)
  Fl::set_color(FL_FOREGROUND_COLOR, 0xd8dee900);

  // Nord1: Fondo de áreas de texto y listas (Azul más profundo)
  Fl::set_color(FL_BACKGROUND2_COLOR, 0x3b425200);

  // Nord8: Color de selección (Azul Frost)
  Fl::set_color(FL_SELECTION_COLOR, 0x88c0d000);
  // -------------------------------
...
" -test\t\t\tTest the window graphics"
  ...
  fltk::load_theme();
  root.show();
 // --- TEMA NORD DEFINITIVO (Ponlo AQUÍ) ---
  Fl::set_color(FL_GRAY, 0x2e344000);             // Nord0: Azul Polar oscuro
  Fl::set_color(FL_FOREGROUND_COLOR, 0xd8dee900); // Nord4: Blanco Nieve
  Fl::set_color(FL_BACKGROUND2_COLOR, 0x3b425200); // Nord1: Fondo oscuro de cajas
  Fl::set_color(FL_SELECTION_COLOR, 0x88c0d000);  // Nord8: Azul Frost (Brillo)
  
  // Esto quita los bordes blancos/grises de los relieves
  Fl::set_color(FL_LIGHT1, 0x3b425200); 
  Fl::set_color(FL_DARK1, 0x24293300);
  // -----------------------------------------
  ...
  if (home) {
      char cmd[512];
      snprintf(cmd, sizeof(cmd), "test -x %s/.flwm/autostart.sh && %s/.flwm/autostart.sh &", home, home);
      system(cmd);
  }
  // -----------------------
